...
import { Filter, Edit2, Check, Settings } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { calculateNodeWidth } from "@/lib/node-utils"

interface FilterNodeProps extends NodeProps {
  onConfigureFilter: (nodeId: string) => void
}

export const FilterNode = memo(({ data, isConnectable, id, onConfigureFilter, selected }: FilterNodeProps) => {
  const [isEditing, setIsEditing] = useState(false)
  const [nodeName, setNodeName] = useState(data.label || "Filter")
  ...
  return (
    <div
      className={`px-4 py-2 shadow-md rounded-md bg-white border-2 ${selected ? "border-blue-500" : "border-yellow-500"}`}
      style={{ width: `${nodeWidth}px`, minWidth: "220px" }}
    >
      <Handle type="target" position={Position.Top} isConnectable={isConnectable} className="w-2 h-2 bg-yellow-500" />
      ...
      <div className="text-xs mt-1 text-gray-500">
        {conditionCount > 0 ? (
          <span>
            {conditionCount} filter condition{conditionCount !== 1 ? "s" : ""}
          </span>
        ) : (

...
import { Webhook, Edit2, Check } from "lucide-react"
...
export const TriggerNode = memo(({ data, isConnectable, selected }: NodeProps) => {
  ...
  const [nodeName, setNodeName] = useState(data.label || "Webhook Trigger")
  ...
  useEffect(() => {
    ...
    setNodeName(data.label || "Webhook Trigger")
  }, [data.label])
  ...
  return (
    <div
      className={`px-4 py-2 shadow-md rounded-md bg-white border-2 ${selected ? "border-blue-500" : "border-blue-500"}`}
    ...
    >
      ...
      <div className="text-xs mt-1 text-gray-500">Workflow Trigger</div>
